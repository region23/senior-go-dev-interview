# Эффективный Go
>
> Переведено при помощи `ChatGPT 4o` 05.03.2025
...
- **Длина строки**: В Go нет ограничения на длину строки. Не переживайте, что код выйдет за пределы перфокарты. Если строка кажется слишком длинной, перенесите её с дополнительным отступом.
...
Если нужно перебрать элементы массива, среза, строки, карты или прочитать данные из канала, можно использовать `range`:
...
Помните, что `make` применяется **только** к картам, срезам и каналам и **не возвращает указатель**. Чтобы получить явный указатель, используйте `new` или берите адрес переменной явно.
...
Карты (maps) — это удобная и мощная встроенная структура данных, которая связывает значения одного типа (ключ, *key*) со значениями другого типа (элемент, *element* или значение, *value*).  
...
Срезы (*slices*) нельзя использовать в качестве ключей карт, потому что для них не определено понятие равенства.  

Как и срезы, карты хранят ссылки на внутреннюю структуру данных. Если передать карту в функцию и изменить её содержимое, изменения будут видны в вызывающем коде.

#### Создание карты

Карты можно создавать с помощью стандартного синтаксиса составных литералов, используя пары "ключ-значение", разделённые двоеточием. Это позволяет легко инициализировать карту.
...
Присваивание значений и доступ к элементам карты синтаксически похожи на работу с массивами и срезами, за исключением того, что индекс (ключ) может быть не только числом.

```go
offset := timeZone["EST"]
```

Если попытаться получить значение по ключу, которого нет в карте, будет возвращено **нулевое значение** для типа элементов карты. Например, если карта содержит целые числа, а ключ отсутствует, вернётся `0`.

### Использование карты как множества (set)

Множество можно реализовать с помощью карты, где типом значений будет `bool`. Чтобы добавить элемент в множество, достаточно установить для ключа значение `true`, а для проверки существования элемента можно использовать обычное индексирование:
...
Иногда важно различать, отсутствует ли ключ в карте или он просто имеет нулевое значение. Например, как понять, содержится ли `"UTC"` в карте, или же результат `0` означает, что такого ключа нет?  
...
Если ключ `tz` присутствует в карте, `seconds` получит соответствующее значение, а `ok` будет `true`.  
...
Если нужно только проверить, присутствует ли ключ в карте, но само значение не важно, можно использовать **пустой идентификатор** (`_`):
...
Для удаления элемента из карты используется встроенная функция `delete()`, которая принимает карту и ключ, который нужно удалить. Это безопасная операция, даже если ключ уже отсутствует в карте.
...
Как и карты (maps), каналы создаются с помощью `make`, а полученное значение действует как ссылка на базовую структуру данных. Если передан необязательный целочисленный параметр, он устанавливает размер буфера для канала. По умолчанию его значение равно нулю, что делает канал неблокирующим (unbuffered) или синхронным.
